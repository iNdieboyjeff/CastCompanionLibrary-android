 ...
 * custom data channel/namespace if an out-of-bound communication is needed).
 * <p>
 * This is a singleton that needs to be "initialized" (by calling <code>initialize()</code>) prior to usage. Subsequent
 * to initialization, an easier way to get access to the singleton class is to call a variant of
 * <code>getInstance()</code>. After initialization, callers can enable a number of features (all features are off by
 ...
 * </ul>
 * Callers can add {@link MiniController} components to their application pages by adding the corresponding widget to
 * their layout xml and then calling <code>addMiniController()</code>. This class manages various states of the remote
 * cast device.Client applications, however, can complement the default behavior of this class by hooking into various
 * callbacks that it provides (see {@link IVideoCastConsumer}). Since the number of these callbacks is usually much
 * larger than what a single application might be interested in, there is a no-op implementation of this interface (see
 * {@link VideoCastConsumerImpl}) that applications can subclass to override only those methods that they are interested
 * in. Since this library depends on the cast functionalities provided by the Google Play services, the library checks
 * to ensure that the right version of that service is installed. It also provides a simple static method
 * <code>checkGooglePlaySevices()</code> that clients can call at an early stage of their applications to provide a
...
public class VideoCastManager extends BaseCastManager implements OnMiniControllerChangedListener, OnFailedListener {
	 ...
	 *            this points to the activity that should be invoked when user clicks on the icon in the
	 ...
	 * @return
	 */
	public static synchronized VideoCastManager initialize(Context context, String applicationId,
	 ...
	 * <code>VideoCastManager
	 * </code> instance. The main purpose of updating context is to enable the library to provide {@link Context}
	 * related functionalities, e.g. it can create an error dialog if needed. This method is preferred over the similar
	 * one without a context argument.
	 * 
	...
	/**
	 * Updates the visibility of the mini controllers. In most cases, clients do not need to use this as the
	 * {@link VideoCastManager} handles the visibility.
	 * 
	 * @param visible
	 */
	 ...
	 * @throws CastException
	 */
	public void incrementVolume(double delta) throws CastException, TransientNetworkDisconnectionException,
			NoConnectionException {
		checkConnectivity();
	...
	/**
	 * Returns the duration of the media that is loaded, in seconds. If there is no connection or if there is no channel
	 * established, this method returns -1.
	 * 
	 * @return
	 * @throws NoConnectionException
	 * @throws TransientNetworkDisconnectionException
	...
	/**
	 * Returns the current (approximate) position of the current media, in seconds. If there is no channel established,
	...
	/*
	 * Starts a service that can last beyond the lifetime of the application to provide notifications. The service
	 * brings itself down when needed. The service will be started only if the notification feature has been enabled
	...
	@Override
	void onApplicationConnected(ApplicationMetadata appMetadata, String applicationStatus, String sessionId,
			boolean wasLaunched) {
	...
	public void loadMedia(MediaInfo media, boolean autoPlay, int position, JSONObject customData)
		...
		mRemoteMediaPlayer.load(mApiClient, media, autoPlay, position, customData).setResultCallback(
				new ResultCallback<RemoteMediaPlayer.MediaChannelResult>() {
	 ...
	 * @throws TransientNetworkDisconnectionException
	 */
	public void play(JSONObject customData) throws CastException, TransientNetworkDisconnectionException,
			NoConnectionException {
		LOGD(TAG, "play()");
		checkConnectivity();
		try {
	 ...
	 * @throws TransientNetworkDisconnectionException
	 */
	public void pause(JSONObject customData) throws CastException, TransientNetworkDisconnectionException,
			NoConnectionException {
	...
	public void seek(int position) throws TransientNetworkDisconnectionException, NoConnectionException {
		...
		mRemoteMediaPlayer.seek(mApiClient, position, RemoteMediaPlayer.RESUME_STATE_UNCHANGED).setResultCallback(
	...
	public void seekAndPlay(int position) throws TransientNetworkDisconnectionException, NoConnectionException {
		...
		mRemoteMediaPlayer.seek(mApiClient, position, RemoteMediaPlayer.RESUME_STATE_PLAY).setResultCallback(
				resultCallback);
	}
	 ...
	 * meaningful if the status is truly <code>MediaStatus.PLAYER_STATE_IDLE
	 * </code>
	 * 
	 * @return
	 */
	 ...
	 *             If the namesapace is empty or null
	 * @throws NoConnectionException
	 *             If no connectivity to the device exists
	 * @throws TransientNetworkDisconnectionException
	 *             If framework is still trying to recover from a possibly transient loss of network
	...
	private void setUpRemoteControl(final MediaInfo info) {
		...
		if (mRemoteControlClientCompat == null) {
			...
			mRemoteControlClientCompat = new RemoteControlClientCompat(PendingIntent.getBroadcast(mContext, 0, intent,
					0));
	...
	/*
	 * Returns the {@link Bitmap} appropriate for the right size image for lock screen. In ICS and JB, the image shown
	 * on the lock screen is a small size bitmap but for KitKat, the image is a full-screen image so we need to
	 * separately handle these two cases.
	 */
	...
	private void updateRemoteControl(boolean playing) {
		...
		try {
			...
			if (mRemoteControlClientCompat != null) {
				mRemoteControlClientCompat.setPlaybackState(playing ? RemoteControlClient.PLAYSTATE_PLAYING
	...
	/*
	 * On ICS and JB, lock screen metadata is one liner: Title - Album Artist - Album. On KitKat, it has two lines:
	...
	private void updateLockScreenMetadata() {
		...
		try {
			...
			mRemoteControlClientCompat
					...
					.putLong(MediaMetadataRetriever.METADATA_KEY_DURATION, info.getStreamDuration()).apply();
	...
	// com.actionbarsherlock.view.MenuItem mediaRouteMenuItem = menu.findItem(menuResourceId);
