 ...
 * {@link VideoCastControllerActivity} (or more generally, for any class implementing {@link IVideoCastController}
 * interface). This can come very handy when setup of that activity allows for a configuration changes. Most of the
 * logic required for {@link VideoCastControllerActivity} is maintained in this fragment to enable application
 * developers provide a different implementation, if desired.
 * <p/>
 * This fragment also provides an implementation of {@link IMediaAuthListener} which can be useful if a
 * pre-authorization is required for playback of a media.
 */
public class VideoCastControllerFragment extends Fragment implements OnVideoCastControllerListener, IMediaAuthListener {
	...
	private void handleMediaAuthTask(final IMediaAuthService authService) {
		...
		mCastController.setLine2(null != authService.getPendingMessage() ? authService.getPendingMessage() : "");
	...
	private void updateOverallState() {
		...
		case AUTHORIZING:
			...
			if (null != authService) {
				mCastController
						.setLine2(null != authService.getPendingMessage() ? authService.getPendingMessage() : "");
				mCastController.showLoading(true);
			}
			break;
		case PLAYBACK:
	...
	private void updateMetadata() {
		...
		mCastController.setLine1(null != mm.getString(MediaMetadata.KEY_TITLE) ? mm.getString(MediaMetadata.KEY_TITLE)
				: "");
	...
	/*
	 * Gets the image at the given url and populates the image view with that. It tries to cache the image to avoid
	...
	/*
	 * A modal dialog with an OK button, where upon clicking on it, will finish the activity. We use a DialogFragment so
	 * during configuration changes, system manages the dialog for us.
	 */
	public static class ErrorDialogFragment extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			return new AlertDialog.Builder(getActivity()).setTitle(R.string.error).setMessage(message)
	...
	@Override
	public void onResult(MediaAuthStatus status, final MediaInfo info, final String message, final int startPoint,
