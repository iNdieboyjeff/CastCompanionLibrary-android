...
public abstract class BaseCastManager implements DeviceSelectionListener, ConnectionCallbacks,
	 ...
	 * <li>User adds the {@link android.support.v7.app.MediaRouteButton} to the layout and pass a reference to that
	 * instance to this method</li>
	 ...
	 * Bitwise OR combination of features should be passed in if multiple features are needed
	 * <p/>
	 ...
	 */
	public void setDeviceVolume(double volume) throws CastException, TransientNetworkDisconnectionException,
	 ...
	 * @throws TransientNetworkDisconnectionException
	 */
	public void incrementDeviceVolume(double delta) throws CastException, TransientNetworkDisconnectionException,
			NoConnectionException {
		checkConnectivity();
	 ...
	 * @throws TransientNetworkDisconnectionException
	 */
	public void setDeviceMute(boolean mute) throws CastException, TransientNetworkDisconnectionException,
			NoConnectionException {
		checkConnectivity();
		try {
	...
	private void reconnectSessionIfPossibleInternal(RouteInfo theRoute) {
		...
		LOGD(TAG, "reconnectSessionIfPossible() Retrieved from preferences: " + "sessionId=" + sessionId + ", routeId="
				+ routeId);
		if (null == sessionId || null == routeId) {
			return;
		}
	 ...
	 * </ul>
	 * Under these conditions, a best-effort attempt will be made to continue with the same session. This attempt will
	 * go on for <code>timeoutInSeconds</code> seconds. During this period, an optional dialog can be shown if
	 * <code>showDialog</code> is set to <code>true</code>. The message in this dialog can be changed by overriding the
	 * resource <code>R.string.session_reconnection_attempt</code>
	 * 
	 * @param context
	 * @param showDialog
	 ...
	 * </ul>
	 * Under these conditions, a best-effort attempt will be made to continue with the same session. This attempt will
	 * go on for 5 seconds. During this period, an optional dialog can be shown if <code>showDialog</code> is set to
	 * <code>true
	 * </code>.
	 * 
	 * @param context
	...
	public void onConnectionFailed(ConnectionResult result) {
		LOGD(TAG,
				"onConnectionFailed() reached, error code: " + result.getErrorCode() + ", reason: " + result.toString());
		mConnectionSuspened = false;
						...
						public void onResult(ApplicationConnectionResult result) {
							if (result.getStatus().isSuccess()) {
								...
								onApplicationConnected(result.getApplicationMetadata(), result.getApplicationStatus(),
										result.getSessionId(), result.getWasLaunched());
							} else {
